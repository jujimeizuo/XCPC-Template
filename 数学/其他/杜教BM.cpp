...
namespace linear_seq {
    ...
    ll solve(ll n, VI a, VI b) {
        ...
        for(ll i = 0;i < k; i++) _md[k - i - 1] = -a[i];
        _md[k] = 1;
        Md.clear();
        for(ll i = 0;i < k; i++) {
        ...
        for(ll p = pnt;p >= 0; p--) {
            ...
            if((n >> p) & 1) {
                ...
                res[0] = 0;
                for(ll i = 0;i < Md.size(); i++) res[Md[i]] = (res[Md[i]] - res[k] * _md[Md[i]]) % mod;
            }
        }
    ...
    VI BM(VI s) {
        ...
        for(ll n = 0;n < s.size(); n++) {
            ...
            if(d == 0) m++;
                ...
                ll c = mod - d * quick_pow(b, mod - 2) % mod;
                while(C.size() < B.size() + m) C.push_back(0);
                for(ll i = 0;i < B.size(); i++) C[i + m] = (C[i + m] + c * B[i]) % mod;
                m++;
            }
        }
        return C;
...
int main() {
    ll n; cin >> n;
    cout << linear_seq::gao(VI{1, 5, 11, 36, 95, 281, 781, 2245, 6336, 18061, 51205}, n - 1) << endl;
}