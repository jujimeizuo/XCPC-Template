...
template <typename T>
typename std::enable_if<std::is_same<typename Modular<T>::Type, int>::value, std::vector<Modular<T>>>::type operator*(
    const std::vector<Modular<T>>& a,
  ...
  std::vector<int> a_mul(a.size());
  for (int i = 0; i < (int) a.size(); i++) {
    a_mul[i] = static_cast<int>(a[i]);
  }
  std::vector<int> b_mul(b.size());
  for (int i = 0; i < (int) b.size(); i++) {
    b_mul[i] = static_cast<int>(b[i]);
  }
  std::vector<int> c_mul = fft::multiply_mod(a_mul, b_mul, T::value);
  std::vector<Modular<T>> c(c_mul.size());
